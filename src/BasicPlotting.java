...
public class BasicPlotting extends StepCounter {
	// ** add methods to Adaptive, change CSV data, comments, Data
	...
	public static void main(String[] args) {
		...
		CSVData raw = new CSVData("data/35Run.csv", columnNames, 1);
		...
		plot.addLinePlot("Raw Data", rawData);
		plot.addLinePlot("Smoothed Data", smoothedData);
		plot.addLinePlot("Adaptive Threshold", adaptiveThresholdData);
		plot.addLinePlot("Smoothed Threshold", smoothedAdaptiveThresholdData);

		System.out.println(
				"StepCounterRevised:" + stepCounterRevised(raw.getCol(1), raw.getRows(1, raw.getNumRows() - 1), 10));
		System.out.println("StepCounter:" + stepCounter(raw.getCol(1), raw.getRows(1, raw.getNumRows() - 1)));
	...
	public static double[] getAdaptiveThreshold2(double[] arr, int windowLength) {
		// calculate mean and standard deviation for each point within a window of certain length
		// use these values to calculate a threshold
		// pay attention to three cases:
		// -when the point is within the window number of points of the left and right interval
		// -when the point is within window number points of the right interval but less than window number 
		//of points from the left interval
		// -when the point is less than window number of points away from the right interval
	...
	public static double[] getAdaptiveThreshold(double[] arr, int windowLength) {
		...
		double[] thresholds = new double[arr.length];
		for (int i = 0; i < arr.length; i++) {
			if (i + windowLength < arr.length && i - windowLength >= 0) {
